...
async function getDietList(req, res) {
  ...
  async function getSugarList(req, res) {
	id = req.user.id;
	console.log(id);
	const result = await plannerRepository.getSugarList(id);
  ...
  async function insertSugar(req, res) {
    const {  mealtime,date, time, notes, sugar } = req.body;
    const userid = req.user.id;
    // let  k = 0;
	// console.log("ejfrfr" + repeat);
    // if(repeat.toLowerCase() === "Does not repeat".toLowerCase()) k = 0;
    ...
    try {
        // Use the to_timestamp function to convert AM/PM time to timestamp
        const result = await plannerRepository.insertSugar(
			userid, mealtime,newDate, time, notes, parseFloat(sugar)
        );
